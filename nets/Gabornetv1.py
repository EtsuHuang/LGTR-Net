...
def limitation(x, v_max=1, v_min=2):
    value = (v_min + v_max*math.exp(-x))/(1 + math.exp(-x))
    return value
...
class MGaborNeck(nn.Module):
    ...
    def forward(self, x):
        ...
        # text_x = x
        # b, c, w, h = text_x.shape
        #
        # if w == 112 and c == 16:
        #     # print("feature generate")
        #     for i in range(c):
        #         text_x = text_x.cpu()
        #         csv = text_x[0, i, :, :].detach().numpy()
        #         # print("csv", csv.shape)
        #         csv = csv.reshape([112, 112])
        #         # print("type_csv", type(csv))
        #         csv = pd.DataFrame(csv)
        #         csv.to_csv(
        #             "E:\\classification-pytorch-main\\classification-pytorch-main\\feature_figure\\csv_paper\\stage_0_figure\\figure_{}.csv".format(
        #                 i))
        # if w == 56 and c == 24:
        #     # print("feature generate")
        #     for i in range(c):
        ...
        #         csv = text_x[0, i, :, :].detach().numpy()
        #         # print("csv", csv.shape)
        #         csv = csv.reshape([56, 56])
        #         # print("type_csv", type(csv))
        #         csv = pd.DataFrame(csv)
        #         csv.to_csv(
        #             "E:\\classification-pytorch-main\\classification-pytorch-main\\feature_figure\\csv_paper\\stage_1_figure\\figure_{}.csv".format(
        #                 i))
        # if w == 28 and c == 48:
        #     # print("feature generate")
        #     for i in range(c):
        ...
        #         csv = text_x[0, i, :, :].detach().numpy()
        #         # print("csv", csv.shape)
        #         csv = csv.reshape([28, 28])
        #         # print("type_csv", type(csv))
        #         csv = pd.DataFrame(csv)
        #         csv.to_csv(
        #             "E:\\classification-pytorch-main\\classification-pytorch-main\\feature_figure\\csv_paper\\stage_2_figure\\figure_{}.csv".format(
        #                 i))
        # if w == 14 and c == 80:
        #     # print("feature generate")
        #     for i in range(c):
        ...
        #         csv = text_x[0, i, :, :].detach().numpy()
        #         # print("csv", csv.shape)
        #         csv = csv.reshape([14, 14])
        #         # print("type_csv", type(csv))
        #         csv = pd.DataFrame(csv)
        #         csv.to_csv(
        #             "E:\\classification-pytorch-main\\classification-pytorch-main\\feature_figure\\csv_paper\\stage_3_figure\\figure_{}.csv".format(
        #                 i))
...
class MGaborNet(nn.Module):
    def __init__(self, cfgs, num_classes=1000, width=1.0, dropout=0.2, round_nearest=8):
        ...
        output_channel = 1280
        self.output_channel = _make_divisible(output_channel * max(1.0, width), round_nearest)
...
def LGTRNetv1(**kwargs):
    ...
    cfgs = [
        ...
        # stage5(有效特征层)
        # 26,26,112 -> 13,13,160
        [[5, 3, 672, 160, 16, 0, 2, 0]],
        [[5, 3, 960, 160, 16, 0, 1, 0],
         [5, 3, 960, 192, 32, 0, 1, 0],
         [5, 3, 960, 192, 32, 0.25, 1, 0]
         ]
    ]
...
def Lgtr_Netv1(pretrained=False, progress=True, num_classes=1000):
    model = LGTRNetv1()
